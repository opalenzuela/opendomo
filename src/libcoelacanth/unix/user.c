...
void user_add(user_t *o, char *username, volatile char *passwd)
{
   ...
   while ((pw = fgetpwent(f))) 
   {
      ...
      {
         str_assign_str(o->error, USER_ERROR_SIZE, "user_add(): user exists");
         return;
      }
   ...
   if(!f)
   {
      str_assign_str(o->error, USER_ERROR_SIZE, 
			"user_add(): cannot open /etc/passwd");
      return;
   }
   ...
   if (putpwent(&p, f) == -1) 
   {
      str_assign_str(o->error, USER_ERROR_SIZE, "user_add(): putpwent() error");
      return;
   }
   ...
   {
      str_assign_str(o->error, USER_ERROR_SIZE, 
			"user_add(): cannot open /etc/shadow");
      return;
   }

   if (putspent(&sp, f) == -1) 
   {
      str_assign_str(o->error, USER_ERROR_SIZE, "user_add(): putspent() error");
      return;
   }
...
void user_del(user_t *o, char *username)
{
   ...
   {
      str_assign_str(o->error, USER_ERROR_SIZE, 
			"user_del() can not remove user from /etc/passwd");
      return;
   }
...
void user_set_password(user_t *o, char *username, volatile char* passwd)
{
   ...
   {
      str_assign_str(o->error, USER_ERROR_SIZE,
			"user_set_password() unknown user");
      return;
   }
   ...
   {
      str_assign_str(o->error, USER_ERROR_SIZE, 
			"user_set_password() cannot modify /etc/shadow");
      return;
   }
   ...
   {
      str_assign_str(o->error, USER_ERROR_SIZE, 
			"user_set_password(): cannot open /etc/shadow");
      return;
   }
